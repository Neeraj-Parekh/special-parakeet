...
>
> **Sources:** Every production-system claim cites a URL retrieved by
> live web search on 2026-08-29. If I could not
...
- **Compromise accepted.** No Flink CEP pattern compilation (that's Gap 1, deferred). No natural-language rule authoring (that's an LLM call; we already have an LLM SDK wired for the copilot — could be extended to rule authoring in 2 extra days).
...
research session. All web searches executed via the `web_search`
function. All page reads executed via the
`page_reader` tool. No URLs invented; all 38+ source URLs returned by the
