...
* **Kill-switch API** ✅ SHIPPED — RBI MRM §3.2 mandate; `POST/GET
  /v1/admin/kill-switch` in `src/api/routes.py` (admin-scoped, audited,
  auto-expiry via `duration_seconds`). Engages a top-of-handler 503
  pre-check on `/risk/score` BEFORE auth/HMAC/model/audit-write (zero
  CPU burn, zero model traffic). backend-killswitch-1.
